...
def predict_labels(test_data, type_dict, model):
    predictions = []
    for word, true_label in test_data:
        predicted_label = word_classifier(word, type_dict, get_conll_types(), model)
        predictions.append((word, true_label, predicted_label))
    return predictions
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line and not line.startswith("-DOCSTART-"):
                tokens = line.split()
                word = tokens[0]  # Word
                ner_tag = tokens[-1]  # True NER tag
                test_data.append((word, ner_tag))
    return test_data

def evaluate(predictions):
    true_labels = [true for _, true, _ in predictions]
    predicted_labels = [pred for _, _, pred in predictions]
...
def main():
    ...
    # Load training data and classify types
    training_data = load_conll_data("conll2003/train.txt")
    type_dict = classify_conll_types(training_data)

    # Load test data
    test_data = load_test_data("conll2003/test.txt")

    # Predict labels for test data
    predictions = predict_labels(test_data, type_dict, model)
